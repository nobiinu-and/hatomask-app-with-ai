...
### Phase 3 & 4: E2Eステップ定義とフロントエンド開発(同時並行)

**シナリオのステップごとに、ステップ定義→フロントエンド実装のサイクルを繰り返す**

8. **現在のシナリオをステップごとに分割する**
   - Given/When/Thenの各ステップを確認
   - 実装する順序を決定(通常はGiven→When→Thenの順)

9. **最初のステップを選択し、以下のサイクルを回す**
   
   a) **ステップ定義の作成**(Red)
   - `e2e/step-definitions/`に該当ステップの定義を追加
   - Playwrightを使って、そのステップの検証コードを記述
   - E2Eテストを実行し、**失敗することを確認する**(Red状態)
...
12. **テストリストの作成**
    - Phase 3 & 4で確定したAPI仕様とビジネスロジックを基にテストリストを作成
...
15. **モックAPIを無効化し、リアルバックエンドAPIを使用する**
16. 現在のシナリオのE2Eテストを再実行し、**成功することを確認する**(Green状態)
17. E2Eテストが失敗する場合は、原因を特定し修正する
    - バックエンドのレスポンス形式がモックと一致しているか確認
    - エラーハンドリングが適切か確認
    - データ型、フィールド名、ネストの深さなどを検証
18. すべてのテスト(ユニット、統合、E2E)が通ることを確認する

### Phase 7: 次のシナリオへ進む
19. **Specの受け入れ基準から次のシナリオを選ぶ**
20. 次のシナリオを`e2e/features/`に追加する
21. **Phase 3 & 4に戻り、ステップ定義→フロントエンド→バックエンドのサイクルを繰り返す**
22. すべてのシナリオが実装されるまで繰り返す
...
23. すべてのシナリオがGreen状態になったことを確認する
24. コーディング規約に従っているか確認する(`CODING_STANDARDS.md`を参照)
25. Linterを実行し、コード品質をチェックする
26. コードレビューを実施する
27. リファクタリングが必要な箇所を改善する
28. すべてのテスト(ユニット、統合、E2E)が通ることを確認する

### Phase 9: ドキュメント更新とデプロイ
29. README.mdやその他のドキュメントを更新する
30. テストリスト(`testlists/`)を更新する
31. 変更をコミット・プッシュする
32. CI/CDパイプラインでテストが通ることを確認する
33. デプロイを実行する
...
2. **Featureファイル作成**: 選んだシナリオをCucumberのFeatureファイルに追加
3. **ステップ定義とフロントエンド実装(同時並行・ステップ単位)**: 
   - シナリオのステップ(Given/When/Then)を1つずつ実装
   - 各ステップで: ステップ定義作成(Red) → モックAPI作成 → ユニットテスト(Red) → 実装(Green) → E2Eテスト(Green) → API検証 → リファクタ
...
- E2Eテストとフロントエンドが同時並行で進み、効率的に開発できる
- デバッグが容易になる
...
**Phase 2: Featureファイル作成**
1. このシナリオをFeatureファイルに追加

**Phase 3 & 4: ステップ定義とフロントエンド実装(ステップ単位)**

**ステップ1: Given - 初期表示**
- ステップ定義作成: Givenステップの検証コード → **Red状態確認**
- モックAPI不要(静的ページ)
- ユニットテスト作成: `App`コンポーネントのレンダリング → **Red状態確認**
- 実装: 基本レイアウトとボタン配置 → **Green**
- E2Eテスト実行(Givenステップまで) → **Green**

**ステップ2: When - ファイル選択**
- ステップ定義作成: Whenステップの検証コード → **Red状態確認**
- モックAPI不要(ブラウザAPI使用)
- ユニットテスト作成: ファイル選択イベント処理 → **Red状態確認**
- 実装: ファイル入力とバリデーション → **Green**
- E2Eテスト実行(Whenステップまで) → **Green**

**ステップ3: And - ファイルアップロード**
- ステップ定義作成: Andステップの検証コード → **Red状態確認**
- **モックAPI作成**: `POST /api/v1/photos`のモックレスポンス
- ユニットテスト作成: アップロードロジック、状態管理 → **Red状態確認**
- 実装: アップロード処理、エラーハンドリング → **Green**
- E2Eテスト実行(Andステップまで) → **Green**
- **API検証**: レスポンス形式が使いやすいか確認、必要に応じて修正

**ステップ4: Then - アップロード成功とプレビュー表示**
- ステップ定義作成: Thenステップの検証コード → **Red状態確認**
- **モックAPI追加**: `GET /api/v1/photos/{id}`のモックレスポンス
- ユニットテスト作成: プレビューコンポーネント → **Red状態確認**
- 実装: 画像表示、レスポンシブ対応 → **Green**
- E2Eテスト実行(Thenステップまで) → **Green**

**ステップ5-6: When/Then - ダウンロード**
- ステップ定義作成: 残りのステップの検証コード → **Red状態確認**
- モックAPI使用(既存のGETエンドポイント)
- ユニットテスト作成: ダウンロード処理 → **Red状態確認**
- 実装: ダウンロードボタンとロジック → **Green**
...
5. **ユニットテスト作成**: ファイル形式の検証ロジック → **Red状態確認**
6. **フロントエンド実装**: ファイル形式の検証ロジック追加 → **Green**
7. E2Eテスト実行(モックAPI使用) → **Green状態確認**
8. **バックエンド実装**: PNG形式のファイル処理追加
## コマンド例
