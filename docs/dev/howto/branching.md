...
このリポジトリでは AI エージェント（Devin、Copilot など）との協働開発が前提です。AI エージェントがブランチを作成する場合の命名規則を説明します。

### 基本方針: 標準命名を使用

AI エージェントが機能開発やバグ修正を行う場合は、**人間の開発者と同じ標準命名規則**を使用します。
...
**例**:
- `feature/01-photo-upload` - AI が写真アップロード機能を実装
- `fix/123-upload-timeout` - AI がバグを修正
- `docs/add-api-spec` - AI がドキュメントを追加

**理由**: 作業の**種類**（feature/fix/docs）が明確になり、誰が作成したかは PR やコミット履歴で追跡できます。

### AI 専用プレフィックスの使用場面

以下の場合に限り、AI 専用プレフィックスを使用します:
...
**例**:
- `devin/1768183963-update-ai-prompts` - AI プロンプトの改善
- `copilot/1768184000-experiment-new-algorithm` - 新アルゴリズムの実験
