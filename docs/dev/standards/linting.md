...
このドキュメントでは、HatoMask プロジェクトの Linter 設定と使用方法について説明します。

Linter は[品質基準](./quality.md)を自動的に強制するためのツールです。コード品質の維持、セキュリティリスクの低減、チーム全体での一貫性確保に役立ちます。
...
- `.eslintrc.cjs`: ESLint 設定ファイル
- `.eslintignore`: ESLint で除外するファイル
...
- ❌ `console.log` は警告 (`console.warn`, `console.error` は OK)
- ❌ `debugger` 禁止
...
- `@typescript-eslint/eslint-plugin`: TypeScript 固有のルール
- `eslint-plugin-react`: React 固有のルール
...
- `checkstyle.xml`: Checkstyle 設定ファイル
...
- ✅ switch 文には`default`必須
- ❌ 空の`catch`ブロック禁止
- ❌ ワイルドカードインポート禁止
- ❌ 未使用のインポート禁止
- ✅ 1 行 1 文 (セミコロンで複数文を書かない)

#### Javadoc

- ⚠️ public クラスとメソッドには Javadoc 推奨 (警告レベル)
...
- ✅ N+1 問題の回避 (JOIN FETCH を使用)
- ✅ ページネーションの実装
- ✅ 適切なインデックスの使用
- ⚠️ ループ内での DB 操作は要レビュー
...
Checkstyle は`validate`フェーズで自動実行されます。
エラーがある場合、ビルドは失敗します。
...
1. **Checkstyle-IDEA プラグインをインストール**
...
2. **設定ファイルを指定**
   ...
   - Configuration File に`checkstyle.xml`を追加
...
GitHub Actions の結果をプルリクエストにコメントとして追加することもできます:
...
1. package-info.java には必ず Javadoc を記述
2. 内部クラスは`@SuppressWarnings("javadoc")`で抑制可能
3. checkstyle.xml で private/protected メソッドは除外
...
この Linter 設定は、以下の品質基準を自動的に強制します:

- **可読性**: 命名規則、コードスタイルの統一
- **保守性**: コードの複雑度制限、ファイル長制限
- **セキュリティ**: 危険なパターンの検出
- **パフォーマンス**: ベストプラクティスの強制
- **アクセシビリティ**: ARIA 属性、セマンティック HTML

詳細は[品質基準](./quality.md)を参照してください。

---

## 関連ドキュメント

- [品質基準](./quality.md) - コード品質の基準とベストプラクティス
