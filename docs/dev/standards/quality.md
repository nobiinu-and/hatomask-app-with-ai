...
- **バックエンド（JaCoCo）**: カバレッジを測定（デフォルトではビルドを失敗させない）
- **フロントエンド（Vitest）**: カバレッジを測定（デフォルトではビルドを失敗させない）
- 必要になったタイミングで、閾値ゲート（失敗条件）を有効化する
